...
bool DSPointLightPassTech::Init(char* pVSFileName, char* pFSFileName)
{
  if (!Technique::Init(pVSFileName, pFSFileName)) {
    return false;
  }

  if (!AddShader(GL_VERTEX_SHADER, pVSFileName)) { // "shaders/light_pass.vs")) {
    return false;
  }


  if (!AddShader(GL_FRAGMENT_SHADER, pFSFileName)) { // "shaders/point_light_pass.fs")) {
    return false;
  }
  ...
  return DSLightPassTech::Init(pVSFileName, pFSFileName);
}
