...
bool DSGeomPassTech::Init(char* pVSFileName, char* pFSFileName)
{
  if (!Technique::Init(pVSFileName, pFSFileName)) {
    return false;
  }

  if (!AddShader(GL_VERTEX_SHADER, pVSFileName)) { // "shaders/geometry_pass.vs")) {
    return false;
  }

  if (!AddShader(GL_FRAGMENT_SHADER, pFSFileName)) { // "shaders/geometry_pass.fs")) {
    return false;
  }
